...
BEGIN
    ...
    BEGIN
        CALL sp_log_error('BOOKING_ERROR', 'Fejl ved oprettelse af booking', CONCAT('Error occurred for booking: ', p_gæste_id), 'HIGH');
        RESIGNAL;
    END;

    IF p_check_ind_dato >= p_check_ud_dato THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Check-ud dato skal være efter check-ind dato';
    END IF;

    SELECT pris INTO v_værelse_pris FROM værelser WHERE hotel_id = p_hotel_id AND værelse_id = p_værelse_id;

    IF v_værelse_pris IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Værelset findes ikke';
    END IF;

    SELECT COUNT(*) = 0 INTO v_er_ledig FROM bookinger 
    WHERE hotel_id = p_hotel_id AND værelse_id = p_værelse_id
    AND booking_status = 'Bekræftet'
    AND ((check_ind_dato BETWEEN p_check_ind_dato AND p_check_ud_dato)
    ...
    IF NOT v_er_ledig THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Værelset er ikke ledigt i den valgte periode';
    END IF;
    ...
    VALUES (p_gæste_id, p_hotel_id, p_værelse_id, p_check_ind_dato, p_check_ud_dato, p_online_booking, p_fdm_medlem, v_total_pris, 'Bekræftet');
...
DROP PROCEDURE IF EXISTS sp_rediger_booking;
...
BEGIN
    ...
    BEGIN
        ROLLBACK;
        CALL sp_log_error('BOOKING_UPDATE_ERROR', 'Fejl ved opdatering af booking', NULL, 'HIGH');
        RESIGNAL;
    END;

    START TRANSACTION;
    
    IF NOT EXISTS (SELECT 1 FROM bookinger WHERE booking_id = p_booking_id) THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Booking findes ikke';
    END IF;
    
    UPDATE bookinger SET 
        gæste_id = p_gæste_id,
        hotel_id = p_hotel_id,
...
CREATE OR REPLACE VIEW v_bookinger AS
SELECT 
    b.*,
